const alunos= ['Elias Miguel']

const AlunosNotas = alunos.map(function(nome){
  return {
    alunos:nome,
    nota:8}
})
console.log("Todos os alunos ")
AlunosNotas.push({nome: 'Maria Antunes',nota:7})
AlunosNotas.push({ nome:'Tito Junior', nota:4})
AlunosNotas.push({nome:'Julio da Sila',nota:6})
AlunosNotas.push({ nome:'Paulo Alberto', nota:5})
console.log(AlunosNotas)
const verificarNota = AlunosNotas.filter(function(item){
  return item.nota >=6
})
...
console.log("Alunos com a nota maior ou igual a 6 ")
const verificarNotaMaiorOuIgual = aluno.filter(function(item){
  return item.nota >=6
});
